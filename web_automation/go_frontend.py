...
class GoBase:
    ...
    def get_player_name(self, player_rank : int) -> str:
        return self.driver.find_element(GoActoinsElements.PLAYER_NAME(player_rank)).text
...
class GoActions(GoBase):
    ...
    def get_player_win_rate(self, player_rank : int) -> int:
        self.driver.click_element_when_visible(GoActoinsElements.PLAYER_INFO_BUTTON(player_rank))
        wins = int(self.driver.find_element(GoActoinsElements.NUMBER_OF_WINS).text)
        total_games = int(self.driver.find_element(GoActoinsElements.TOTAL_GAMES_PLAYED).text)
